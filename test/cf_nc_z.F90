...
program cf_nc_z
    ...
    type(SNCFile) :: file
    type(SNCVar) :: var
    integer, parameter :: N_LAT = 3, N_LON = 4, N_LEV = 2, N_TIME = 2
    integer :: i, j, k, l
    real*4 :: lat(N_LAT), lon(N_LON), lev(N_LEV), time(N_TIME)
    real*4 :: f, t(N_LON, N_LAT, N_LEV, N_TIME)
    real*4, pointer :: t2(:,:,:,:), coord(:)
    integer :: lat_size, lon_size, lev_size, time_size
    ...
    file = snc_cf_grid_create("cfz.nc", N_LON, N_LAT, time_units, &
        SNC_STANDARD_CAL, title = "test file", &
        source = "some test program", src_file = __FILE__, src_line = __LINE__)
    call snc_cf_grid_vertical(file, N_LEV, "lev", "Levels", "hPa", "down", &
    ...
    if (lev_size /= N_LEV) print *, "Bad lev size", lev_size
    ...
    var =  snc_inq_var(file, "lev")
    if (var%ndims /= 1) print *, "Bad lev dim count", var%ndims
    if (var%dims(1) /= lev_size) print *, "Bad lev var size", var%dims(1)

    call snc_get_att(file, var, "long_name", attv)
    if (trim(attv) /= "Levels") print *, "Bad lev:long_name value", attv
    call snc_get_att(file, var, "units", attv)
    if (trim(attv) /= "hPa") print *, "Bad lev:units value", attv
    call snc_get_att(file, var, "axis", attv)
    if (trim(attv) /= "Z") print *, "Bad lev:axis value", attv
    ...
    if (var%ndims /= 4) print *, "Bad t dim count", var%ndims
    if (var%dims(1) /= N_LON) print *, "Bad t dimension 1", var%dims(1)
    if (var%dims(2) /= N_LAT) print *, "Bad t dimension 2", var%dims(2)
    if (var%dims(3) /= N_LEV) print *, "Bad t dimension 3", var%dims(3)
    if (var%dims(4) /= N_TIME) print *, "Bad t dimension 4", var%dims(4)
